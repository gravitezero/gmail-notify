...
GmailButton.prototype = {
	...
	_showNoMessage : function() {
		...
		try {
			...
			let msg = new GmailMenuItem(note,{reactive: true, style_class: 'gn_item'
                           });
		...
		if (typeof(content) != 'undefined'){
			...
			if (content.length>0){
				...
				for (let k=0;k<Math.min(content.length,10);k++){
					let msg = new GmailMenuItem(content[k],{reactive: true, style_class: 'gn_item'
                           });
					msg.connect('activate', _showHello);
					this.menu.addMenuItem(msg,0);
					this.msgs.push(msg);
...
GmailMenuItem.prototype = {
    ...
    _init: function (content, params) {
        ...
        let addr = '';
        try {
			let from_parts = content.from.split(' <');
			from += from_parts[0];
			if (from_parts.length > 0)
				addr += from_parts[1].split('>')[0];
		}
		catch (err) {
			global.log('From field parsing error in gmail menu item proto');
		}

        let from_label = new St.Label({ text: from, style_class: 'from'});
        header_layout.add(from_label);

        let addr_label = new St.Label({ text: addr, style_class: 'addr'});
        header_layout.add(addr_label);

        let date = '';
        let time = '';
        try {
			let date_parts = new Date(content.date).toString().split(' ');
			let time_parts = date_parts[4].split(':');
			date += date_parts[0] + ' ' + date_parts[2] + ' ' + date_parts[1]
        ...
        let time_label = new St.Label({ text: time, style_class:'time'});
        header_layout.add(time_label);

        let date_label = new St.Label({ text: date, style_class:'date'});
        header_layout.add(date_label);

        layout.add(header_layout);

        
        if (_DEBUG) global.log('header added');
        ...
        try
        {
			subtext+=content.subject.length >70 ?  content.subject.substr(0,50)+'...' : content.subject;
		}
		catch (err){
			global.log('Subject converison error in gmail menu item proto'+err.message);
		}
        let content_label = new St.Label({ text:  subtext, style_class: 'content' });


        layout.add(content_label);
...
function show() {
	try{
		...
		switch (config._position) {
			case 0: Main.panel.addToStatusArea('gmail-notify',button,0,'right');
					break;
			case 1: Main.panel.addToStatusArea('gmail-notify',button,0,'center');			
					break;
			case 2: Main.panel.addToStatusArea('gmail-notify',button,0,'left');
					break;
